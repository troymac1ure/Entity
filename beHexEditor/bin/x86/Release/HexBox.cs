...
namespace Be.Windows.Forms
{
	...
	public class HexBox : Control
	{
        ...
        List<SelectionStruct> Selections = new List<SelectionStruct>();
		/// <summary>
		...
		BytePositionInfo GetHexBytePositionInfo(Point p)
		{
			...
			float x = ((float)(p.X - _recHex.X) / _charSize.Width) + 1.5f;
		...
		void PaintHexAndStringView(Graphics g, long startByte, long endByte)
		{
			...
			for(long i = startByte; i < intern_endByte+1; i++)
			{
                Brush multiSelect = null;
                for (int ii = 0; ii < Selections.Count; ii++)
                    if (i >= Selections[ii].selectionStart && i <= Selections[ii].selectionEnd)
                    {
                        multiSelect = new SolidBrush(Selections[ii].foreColor);
                    }
                ...
                {
                    PaintHexString(g, b, multiSelect, gridPoint);
                }
                else
				{
					PaintHexString(g, b, brush, gridPoint);
		...
		public Color SelectionForeColor
		{
            ...
            { 
                ...
                if (SelectionLength > 0)
                {
                    ...
                    {
                        SelectionStruct s = Selections[i];
                        long sLength = s.selectionEnd - s.selectionStart;
                        if (s.selectionStart == SelectionStart && sLength == SelectionLength - 1)
                        {
                            s.foreColor = _selectionForeColor;
                            break;
                        }
                        else
                        {
                            SelectionStruct newSelect = new SelectionStruct(
                                                        SelectionStart,
                                                        sLength,
                                                        _selectionForeColor);
                            if (s.selectionStart == SelectionStart && sLength < SelectionLength - 1)
                            {
                                Selections.Insert(i, newSelect);
                                s.selectionStart = newSelect.selectionEnd + 1;
                                break;
                            }
                            else if (s.selectionStart > SelectionStart && sLength < SelectionLength - 1)
                            {
                                s.selectionEnd = SelectionStart - 1;
                                Selections.Insert(i + 1, newSelect);
                                Selections.Insert(i + 2, new SelectionStruct(newSelect.selectionEnd + 1, SelectionStart + SelectionLength - 1, s.foreColor));
                                s.selectionStart = newSelect.selectionEnd + 1;
                                break;
                            }
                            else if (s.selectionStart > SelectionStart && sLength == SelectionLength - 1)
                            {
                                Selections.Insert(i + 1, newSelect);
                                s.selectionEnd = newSelect.selectionStart - 1;
                                break;
                            }
                            else
                            {
                                // Never finished writing tests for overlap, etc
                                // Feel free to do it if you wish
                            }
                        }
                    }
                    if (_selectionForeColor != Color.Black)
                    {
                        Selections.Add(new SelectionStruct(
                                    SelectionStart,
