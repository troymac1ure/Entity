    ...
    public partial class MetaEditorControlPage : UserControl
    {
        ...
        */
        ...
        public MetaEditorControlPage(Meta meta, MapForms.MapForm mapForm)
        {
            ...
            // Create our default reading / writing streams
            BR = new BinaryReader(meta.MS);
            BW = new BinaryWriter(meta.MS);
        ...
        {
            ifp = HaloMap.Plugins.IFPHashMap.GetIfp(meta.type, map);
        ...
        private void treeViewTagReflexives_DoubleClick(object sender, EventArgs e)
        {
            ...
            int tempTabNum = wME.addNewTab(map.SelectedMeta);
        ...
        private void jumpToTagToolStripMenuItem_Click(object sender, EventArgs e)
        {
            ...
            mf.selectTag(tagNum);
            ((WinMetaEditor)this.TopLevelControl).addNewTab(map.SelectedMeta);
        }
        ...
        {
            ToolStripComboBox tsbc = ((ToolStripComboBox)sender);
            int desiredWidth = 49;
            for (int i = 0; i < tsbc.Items.Count; i++)
                if (TextRenderer.MeasureText(((object)tsbc.Items[i]).ToString(), tsbc.Font).Width + 8 > desiredWidth)
                    desiredWidth = TextRenderer.MeasureText(((object)tsbc.Items[i]).ToString(), tsbc.Font).Width + 16;
            tsbc.Size = new Size(desiredWidth, 21);
        }
        ...
        private void debugPokeValue()
        {
            reflexiveData rd = (reflexiveData)treeViewTagReflexives.SelectedNode.Tag;
#if DEBUG
            Byte[] bytes2 = new byte[CurrentControl.size];
            bytes2 = (byte[])HaloMap.RealTimeHalo.RTH_Imports.Peek(
                (uint)(CurrentControl.offsetInMap + CurrentControl.meta.magic),
                (uint)CurrentControl.size);
            ...
            // Idents must be poked individually with Tag Type, the Tag Name. It doesn't change if both
            // are poked at once.
        ...
        private void tsbtnPoke_Click(object sender, EventArgs e)
        {
            ...
            // Causes data to be updated when control loses focus
            //btnSave.Focus();
            //CurrentControl.Focus();
